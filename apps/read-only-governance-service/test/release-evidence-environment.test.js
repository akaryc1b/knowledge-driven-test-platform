...
} from '../../../scripts/validate-m2-production-promotion-entry.js';
...
test('all release evidence generators resolve main in a push environment', async () => {
  ...
  try {
    ...
    const promotion = await validateM2ProductionPromotion({ generatedAt: GENERATED_AT, commitSha: 'local' });
    assert.equal(m1.source.branch, 'main');
    assert.equal(m2.source.branch, 'main');
    assert.equal(postMerge.source.branch, 'main');
    assert.equal(promotion.source.branch, 'main');
  } finally {
