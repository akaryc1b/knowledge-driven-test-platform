...
- PR #22 已合并；
- R0 merge commit `edf09333d9be9ea6839b8cf4d18efed95cfba821` 的 push run `30423781549` 已通过；
- Production Promotion 已由永久 closure 证据关闭 `main-branch-final-ci-not-verified`；
- 仍有五项外部 blocker 开放，`productionEligible=false`。
...
R1-A 允许选择包含该发布 Workflow 的后续 `main` 提交。R1-B 必须将真实发布 source SHA、其最终 main-CI 证据和 Registry digest 重新绑定到 Production Promotion，不得保留与镜像来源不一致的旧 source SHA。
