...
| PP-03 | 精确晋级源 | 40 位 Git SHA 与固定 `main` SHA | 通过：`edf09333...` | `promotionSource.mainSha` 精确匹配 |
| PP-04 | 最终 `main` push CI | exact run/source、Validate/PostgreSQL jobs、Deployment step、六类 Artifact digest | 通过：run `30423781549` | 所有字段完整、成功且不含占位值 |
| PP-05 | GHCR 不可变镜像 | `@sha256:` 引用与 Registry digest 同值 | 缺失 | build/pull/SBOM/Attestation 证据完整 |
| PP-06 | 本地 Image ID 禁止冒充 | 拒绝 `sha256:` 以外的 image reference 与 `docker-image://` | 通过 | 仅接受 Registry digest |
...
| PP-17 | 占位证据禁止 | placeholder/repeated-hex 检测 | 通过 | 关闭证据不可为示例或伪造格式 |
...
| PP-20 | R0 main-CI closure | `r0-main-ci-closure.json` 精确 run/job/step/artifact Validator | 通过 | run `30423781549` 与 Production Promotion 完全一致 |
...
1. `external-registry-digest-missing`
2. `production-secrets-not-configured`
3. `target-cluster-validation-not-run`
4. `change-approval-missing`
5. `release-owner-approval-missing`
