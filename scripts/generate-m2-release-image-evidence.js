...
export async function generateM2ReleaseImageEvidence(options = {}) {
  ...
  assert(releaseId === promotion.releaseId && releaseId === 'M2-RC1', 'Release ID does not match Production Promotion');
...
function attestationFromEnv(env, prefix, label) {
  ...
  const bundleDigest = requiredDigest(env, `KDTP_RELEASE_${prefix}_BUNDLE_DIGEST`, `${label} bundle digest`);
...
if (isMain) process.stdout.write(`${JSON.stringify(await generateM2ReleaseImageEvidence(), null, 2)}\n`);
