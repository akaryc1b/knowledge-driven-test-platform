import {
  KNOWLEDGE_QUERY_PARAMETERS,
  SNAPSHOT_QUERY_PARAMETERS,
} from './constants.js';
import { HttpBoundaryError, httpInvariant } from './errors.js';

const ROUTES = Object.freeze([
  route('listKnowledge', /^\/v1\/projects\/([^/]+)\/knowledge$/, KNOWLEDGE_QUERY_PARAMETERS),
  route('getReviewTimeline', /^\/v1\/projects\/([^/]+)\/knowledge\/([^/]+)\/versions\/([^/]+)\/timeline$/, []),
  route('getKnowledge', /^\/v1\/projects\/([^/]+)\/knowledge\/([^/]+)\/versions\/([^/]+)$/, []),
  route('listSnapshots', /^\/v1\/projects\/([^/]+)\/snapshots$/, SNAPSHOT_QUERY_PARAMETERS),
  route('getSnapshot', /^\/v1\/projects\/([^/]+)\/snapshots\/([^/]+)$/, []),
]);

export function matchReadOnlyRoute(method, inputUrl, maxUrlLength = 4096) {
  ...
  if (!routeMatch) throw new HttpBoundaryError('ROUTE_NOT_FOUND', 'Read-only route was not found', 404);
  if (method !== 'GET') {
  ...
  const [projectId, first, second] = segments;
  const params = routeMatch.handler === 'getSnapshot'
    ? { snapshotId: first }
    : routeMatch.handler === 'getKnowledge' || routeMatch.handler === 'getReviewTimeline'
      ? { id: first, version: second }
      : {};
  return { handler: routeMatch.handler, projectId, params, query };
}
