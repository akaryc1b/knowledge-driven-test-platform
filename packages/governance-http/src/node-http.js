...
import { mapHttpBoundaryError } from './transport.js';
import { HttpBoundaryError } from './errors.js';
import { normalizeHttpHeaders, resolveRequestId } from './request.js';

export function createReadOnlyNodeHttpHandler({ transport }) {
  if (!transport || typeof transport.dispatch !== 'function') {
    throw new TypeError('transport.dispatch must be a function');
  }
  return async function readOnlyNodeHttpHandler(request, response) {
    let requestId = null;
    try {
      ...
      writeNodeResponse(response, mapHttpBoundaryError(error, requestId));
    }
  };
}

export function createReadOnlyNodeHttpServer(options) {
  return createServer(createReadOnlyNodeHttpHandler(options));
}
