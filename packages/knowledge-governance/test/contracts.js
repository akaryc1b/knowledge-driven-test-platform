...
export function defineAuthorizationContract(test, createAuthorization) {
  test('authorization adapter contract: project grants are isolated and defensive', async () => {
...
export function defineReviewStoreContract(test, createStore) {
  test('review store adapter contract: decisions are append-only and queryable by revision', async () => {
    const store = await createStore();
    const first = decision({ decisionId: 'decision:contract:0001', reviewRevision: 2 });
    const second = decision({
      decisionId: 'decision:contract:0002',
      reviewRevision: 5,
      reviewer: 'reviewer-2',
    });
    ...
    assert.deepEqual(
      (await store.list({ projectId: 'approval-platform', reviewRevision: 5 })).map((item) => item.decisionId),
      ['decision:contract:0002'],
    );
    const listed = await store.list({ projectId: 'approval-platform' });
    listed[0].reason = 'mutated';
    assert.equal((await store.list({ projectId: 'approval-platform' }))[0].reason, 'contract approval');
  });
}

export function defineSnapshotStoreContract(test, createStore) {
  test('snapshot store adapter contract: immutable ID and project filters', async () => {
    const store = await createStore();
