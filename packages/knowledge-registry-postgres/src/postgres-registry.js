...
import { assertPostgresPool } from './pool-contract.js';
...
export class PostgresKnowledgeRegistry extends KnowledgeRegistryPort {
  /** @param {{pool: unknown}} input */
  constructor(input) {
    super();
    this.pool = assertPostgresPool(input?.pool);
  }
    ...
    try {
      return await withPostgresTransaction(this.pool, async (client) => {
        await client.query('SELECT pg_advisory_xact_lock(hashtextextended($1, 0))', [
          `knowledge-id:${record.knowledge.id}`,
        ]);

        const duplicate = await client.query(
          `SELECT 1 FROM ${POSTGRES_SCHEMA}.knowledge_records
             WHERE knowledge_id = $1 AND knowledge_version = $2`,
          [record.knowledge.id, record.knowledge.version],
        );
        if (duplicate.rowCount > 0) {
          throw new RegistryError(
            'KNOWLEDGE_VERSION_EXISTS',
            `Knowledge version ${record.key} already exists`,
            { key: record.key },
          );
        }
        ...
        const highest = await client.query(
          `SELECT knowledge_version
             FROM ${POSTGRES_SCHEMA}.knowledge_records
        ...
        await client.query(
          ...
          [
            ...
            parsedVersion.minor,
            parsedVersion.patch,
            record.knowledge.status,
            record.knowledge.scope.level,
            record.knowledge.scope.key,
            record.revision,
            record.createdAt,
            record.updatedAt,
  ...
  async getLatestPublished(query) {
    ...
    try {
      const records = await this.readRecords({
        filter: { id, status: 'PUBLISHED' },
        descendingVersion: true,
        limit: 1,
      });
      return records[0] ?? null;
    } catch (error) {
  ...
  /**
   * @param {{recordKey?: string, filter?: Record<string, unknown>, descendingVersion?: boolean, limit?: number}} options
   */
...
async function requireRecordForUpdate(client, key) {
  ...
  const records = await hydrateRows(client, result.rows);
  return records[0];
}
  ...
  const updated = await client.query(
    `UPDATE ${POSTGRES_SCHEMA}.knowledge_records
      ...
      WHERE record_key = $7
        AND revision = $8`,
    [
      next.knowledge.status,
      next.knowledge.scope.level,
      next.knowledge.scope.key,
      next.revision,
      next.updatedAt,
...
function buildRecordSelect(options) {
  ...
  const versionDirection = options.descendingVersion ? 'DESC' : 'ASC';
  let text = `SELECT ${RECORD_COLUMNS}
                FROM ${POSTGRES_SCHEMA}.knowledge_records
                ${where}
                ORDER BY knowledge_id ASC,
                         version_major ${versionDirection},
                         version_minor ${versionDirection},
                         version_patch ${versionDirection}`;
